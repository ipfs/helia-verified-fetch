...
import { CONTENT_TYPE_CAR, CONTENT_TYPE_IPNS, CONTENT_TYPE_JSON, CONTENT_TYPE_RAW, CONTENT_TYPE_TAR } from '../src/utils/content-types.ts'
import { getETag } from '../src/utils/get-e-tag.ts'
...
describe('getETag', () => {
  it('CID eTag', () => {
    expect(getETag({ cid: testCID, contentType: CONTENT_TYPE_IPNS })).to.equal(`W/"${cidString}.bin"`)
    expect(getETag({ cid: testCID, contentType: CONTENT_TYPE_RAW })).to.equal(`"${cidString}.raw"`)
  })

  it('should return ETag with CID and format suffix', () => {
    expect(getETag({ cid: testCID, contentType: CONTENT_TYPE_RAW })).to.equal(`"${cidString}.raw"`)
    expect(getETag({ cid: testCID, contentType: CONTENT_TYPE_JSON })).to.equal(`"${cidString}.json"`)
  })
